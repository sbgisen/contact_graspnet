...
from contact_graspnet_ros.srv import Inference, InferenceResponse, InferenceRequest
...
class GraspInference(object):
    ...
    def inference(self, req: InferenceRequest):
        ...
        for seg in pc_segments.values():
            seg *= scale
        pred_grasps_cam, scores, contact_pts, _ = self.grasp_estimator.predict_scene_grasps(self.sess, pc_full, pc_segments=pc_segments, filter_grasps=True)
        ...
        for mat, score in zip(pred_grasps_cam.values(), scores.values()):
            ...
            for i in range(mat.shape[0]):
                ...
                pose.orientation = Quaternion(*quaternion_multiply(quaternion_from_matrix(mat[i]),quaternion_from_euler(0, 0, np.pi/2)))
    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.inferenced:
                ...
                visualize_grasps(self.pc_full, self.pred_grasps_cam, self.scores, plot_opencv_cam=True, pc_colors=self.pc_colors)
            rospy.sleep(1)
